export async function generateDockerToken(username: string): Promise<string> {
  ...
  try {
    ...
    const tokenResponse = await fetch(`https://hub.docker.com/v2/users/${dockerHubUsername}/tokens/`, {
      method: "POST",
      headers: {
    ...
    throw new Error(`Failed to generate Docker token: ${error instanceof Error ? error.message : "Unknown error"}`)
  }
}
